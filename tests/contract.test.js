...
test("template makes marketplace presentation requirements impossible to overlook", () => {
  ...
  const templateFixture = path.join(root, "e2e/bin/curl")
  assert.equal(fs.statSync(templateFixture).mode & 0o111, 0o111)
  assert.match(read("e2e/bin/curl"), /"SHELL RENDER","value":"REAL"/)
  assert.match(read("e2e/bin/curl"), /"VISUAL REVIEW","value":"REQUIRED"/)
